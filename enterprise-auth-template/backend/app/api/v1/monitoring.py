...
async def get_health_status(
    ...
    return StandardResponse(
        success=True, data=health, message="System health retrieved successfully"
    )


@router.get(
    "/metrics", response_class=PlainTextResponse, summary="Get Prometheus metrics"
)
async def get_prometheus_metrics() -> PlainTextResponse:
    ...
    return PlainTextResponse(
        content=metrics.decode("utf-8"), media_type="text/plain; charset=utf-8"
    )
...
async def detect_anomalies(
    ...
    return StandardResponse(
        success=True, data=anomalies, message=f"Found {len(anomalies)} anomalies"
    )
...
async def send_alert(
    ...
    return StandardResponse(
        success=True, data={"status": "sent"}, message="Alert sent successfully"
    )
...
async def generate_report(
    ...
    return StandardResponse(
        success=True, data=report, message="Report generated successfully"
    )
...
async def track_event(
    ...
    return StandardResponse(
        success=True, data={"status": "tracked"}, message="Event tracked successfully"
    )
...
async def test_alert_channels(
    ...
    for channel in channels:
        try:
            await monitoring_service.send_alert(
                ...
                channels=[channel],
                metadata={"test": True, "initiated_by": str(current_user.id)},
            )
    ...
    return StandardResponse(
        success=True, data=results, message="Alert channel test completed"
    )
