...
class SecurityMetricsResponse(BaseModel):
    ...
    password_breach_detections: int = Field(
        ..., description="Password breach detections"
    )
...
async def get_metrics_overview(
    current_user: CurrentUser = Depends(
        require_permissions(["metrics:read", "admin:access"])
    ),
...
async def get_user_analytics(
    time_range: TimeRange = Query(
        TimeRange.WEEK, description="Time range for analysis"
    ),
    current_user: CurrentUser = Depends(
        require_permissions(["metrics:read", "users:read"])
    ),
    db: AsyncSession = Depends(get_db_session),
    ...
    """
    logger.info(
        "User analytics requested", user_id=current_user.id, time_range=time_range
    )
...
async def get_authentication_metrics(
    ...
    current_user: CurrentUser = Depends(
        require_permissions(["metrics:read", "security:read"])
    ),
    db: AsyncSession = Depends(get_db_session),
...
async def get_api_usage_metrics(
    ...
    current_user: CurrentUser = Depends(
        require_permissions(["metrics:read", "api:read"])
    ),
    db: AsyncSession = Depends(get_db_session),
    ...
    """
    logger.info(
        "API usage metrics requested", user_id=current_user.id, time_range=time_range
    )
...
async def get_security_metrics(
    ...
    """
    logger.info(
        "Security metrics requested", user_id=current_user.id, time_range=time_range
    )
    ...
    try:
        ...
        return SecurityMetricsResponse(
            ...
            password_breach_detections=security_data.get(
                "password_breach_detections", 0
            ),
...
async def get_system_performance_metrics(
    ...
    current_user: CurrentUser = Depends(
        require_permissions(["metrics:read", "system:read"])
    ),
    db: AsyncSession = Depends(get_db_session),
    ...
    """
    logger.info(
        "Performance metrics requested", user_id=current_user.id, time_range=time_range
    )
