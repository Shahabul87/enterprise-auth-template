...
class TestRateLimiter:
    ...
    async def test_check_rate_limit_within_limit(
        self, rate_limiter: RateLimiter, mock_redis_client: MagicMock
    ) -> None:
        ...
        allowed, remaining, reset_time = await rate_limiter.check_rate_limit(
            key="test:key", limit=10, window=60
        )
    ...
    def test_get_client_identifier_with_user(self, rate_limiter: RateLimiter) -> None:
    ...
    def test_get_client_identifier_with_ip(self, rate_limiter: RateLimiter) -> None:
...
class TestRateLimitMiddleware:
    ...
    async def test_middleware_allows_request(
        self, test_app: FastAPI, mock_redis_client: MagicMock
    ) -> None:
        ...
        with patch("app.middleware.rate_limiter.redis.from_url") as mock_from_url:
            ...
            test_app.add_middleware(
                RateLimitMiddleware, redis_url="redis://localhost:6379"
            )
    ...
    def test_middleware_disabled_without_redis(self, test_app: FastAPI) -> None:
        """Test middleware is disabled when Redis not configured."""
        # Add middleware without Redis URL
        test_app.add_middleware(RateLimitMiddleware, redis_url=None)
