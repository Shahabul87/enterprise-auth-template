...
async def setup_two_factor(
    ...
    try:
        ...
        logger.info("2FA setup initiated successfully", user_id=current_user.id)

        return TwoFactorSetupResponse(**setup_data)

    except TwoFactorError as e:
        logger.warning("2FA setup failed", user_id=current_user.id, error=str(e))
...
async def enable_two_factor(
    ...
    try:
        ...
        logger.info("2FA enabled successfully", user_id=current_user.id)
        ...
        logger.warning("2FA enable failed", user_id=current_user.id, error=str(e))
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error(
...
async def verify_two_factor(
    ...
    try:
        ...
        verified = await service.verify_2fa_code(
            current_user, request.code, request.is_backup
        )
        ...
        logger.warning("2FA verification failed", user_id=current_user.id, error=str(e))
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error(
...
async def disable_two_factor(
    ...
    try:
        ...
        logger.info("2FA disabled successfully", user_id=current_user.id)

        return MessageResponse(message="Two-factor authentication has been disabled")

    except TwoFactorError as e:
        logger.warning("2FA disable failed", user_id=current_user.id, error=str(e))
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error(
...
async def regenerate_backup_codes(
    ...
    try:
        ...
        logger.info("Backup codes regenerated successfully", user_id=current_user.id)
...
async def request_two_factor_recovery(
    request: RecoveryRequest, db: AsyncSession = Depends(get_db_session)
) -> MessageResponse:
    """
    ...
    if not user or not user.two_factor_enabled:
        ...
        try:
            ...
            logger.info("2FA recovery email sent", user_id=user.id, email=request.email)
        except Exception as e:
            logger.error(
    ...
    return MessageResponse(
        message=(
            "If an account exists with this email and has 2FA enabled, "
