...
failed_logins = Counter(
    "failed_login_attempts_total", "Total failed login attempts", ["reason"]
)

security_events = Counter(
    "security_events_total", "Security events by type", ["event_type", "severity"]
)

api_errors = Counter(
    "api_errors_total", "API errors by type", ["error_type", "endpoint"]
)

database_connections = Gauge(
    "database_connections", "Number of database connections", ["state"]
)

cache_operations = Counter(
    "cache_operations_total", "Cache operations", ["operation", "status"]
)
...
class MonitoringService:
    ...
    async def track_event(
        ...
        await self.redis_client.setex(
            event_key, 86400, json.dumps(event_data)  # 24 hour TTL
        )

        # Update metrics
        security_events.labels(event_type=event_type, severity=severity.value).inc()
    ...
    async def track_authentication(
        ...
        auth_attempts.labels(
            method=method, status="success" if success else "failed"
        ).inc()
        ...
        if not success:
            ...
            await self._track_failed_attempt_pattern(user_id, ip_address, device_id)

    async def track_api_error(
        self, error_type: str, endpoint: str, status_code: int, error_message: str
    ) -> None:
        """Track API errors"""
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        health_score = self._calculate_health_score(
            db_health, redis_health, auth_health
        )
    ...
    async def get_current_metrics(self) -> Dict[str, Any]:
        ...
        auth_stats = await self.db.execute(
            select(
                ...
                func.sum(
                    func.cast(AuditLog.event_type.like("%success%"), Integer)
                ).label("successful"),
            ).where(
                and_(
    ...
    async def send_alert(
        ...
        # Log alert
        logger.warning(
            "Alert sent", title=title, severity=severity.value, channels=channels
        )
    ...
    def _calculate_health_score(self, *health_checks) -> float:
        """Calculate overall health score"""
        scores = {"healthy": 100, "degraded": 60, "unhealthy": 0}
    ...
    async def _send_alert_to_channel(
        self, channel: str, alert_data: Dict[str, Any]
    ) -> None:
