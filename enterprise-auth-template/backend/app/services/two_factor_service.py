...
class TwoFactorService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        # SECURITY: This MUST come from secure key management in production
        encryption_key = getattr(settings, "ENCRYPTION_KEY", None)
        if not encryption_key:
            if settings.ENVIRONMENT == "production":
                logger.critical(
                    "ENCRYPTION_KEY not configured in production - "
                    "2FA backup codes cannot be encrypted"
                )
                raise ValueError(
                ...
                logger.warning(
                    "Using generated ENCRYPTION_KEY for development - "
                    "configure ENCRYPTION_KEY environment variable for production"
                )
        ...
        try:
            ...
            logger.critical("Invalid ENCRYPTION_KEY format", error=str(e))
            raise ValueError(
                "Invalid ENCRYPTION_KEY format. Must be a valid Fernet key."
            )
    ...
    async def setup_totp(self, user: User) -> Dict[str, Any]:
        ...
        encrypted_codes = self.cipher.encrypt(
        ...
        logger.info("TOTP setup initiated", user_id=user.id, email=user.email)
    ...
    async def verify_2fa_code(
        self, user: User, code: str, is_backup: bool = False
    ) -> bool:
        """
        ...
        # Check if user is locked out with exponential backoff
        if user.failed_2fa_attempts >= self.MAX_2FA_ATTEMPTS:
            # Exponential backoff: base delay * 2^(attempts - max_attempts)
            backoff_multiplier = 2 ** max(
                0, user.failed_2fa_attempts - self.MAX_2FA_ATTEMPTS
            )
            lockout_duration = min(
                self.LOCKOUT_DURATION * backoff_multiplier, 3600
            )  # Max 1 hour

            base_time = user.two_factor_verified_at or datetime.utcnow()
            lockout_time = base_time + timedelta(seconds=lockout_duration)
            if datetime.utcnow() < lockout_time:
                # Log suspicious activity
                logger.warning(
                    "2FA brute force attempt detected",
                    user_id=user.id,
                ...
                raise TwoFactorError(
                    ...
                    f"Try again after {lockout_time.strftime('%H:%M UTC')}"
                )
            else:
    ...
    async def _verify_backup_code(self, user: User, code: str) -> bool:
        ...
        try:
            ...
            for i, hashed_code in enumerate(hashed_codes):
                if hashed_code and verify_password(code, hashed_code):
                    ...
                    logger.info(
                        ...
                        codes_remaining=self.BACKUP_CODES_COUNT
                        - user.two_factor_recovery_codes_used,
                    )
            ...
            logger.error(
                "Backup code verification error", user_id=user.id, error=str(e)
            )
            return False
    ...
    async def disable_two_factor(self, user: User, password: str) -> bool:
        ...
        logger.info("Two-factor authentication disabled", user_id=user.id)
    ...
    async def get_2fa_status(self, user: User) -> Dict[str, Any]:
        ...
        if user.two_factor_enabled and user.backup_codes:
            backup_codes_remaining = (
                self.BACKUP_CODES_COUNT - user.two_factor_recovery_codes_used
            )

        return {
            "enabled": user.two_factor_enabled,
            "verified_at": (
                user.two_factor_verified_at.isoformat()
    ...
    def _generate_backup_codes(self) -> List[str]:
        ...
        Returns:
            List[str]: Generated backup codes with high entropy
        """
        codes = []
        # Use larger character set for higher entropy
        charset = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"

        for _ in range(self.BACKUP_CODES_COUNT):
            # Generate cryptographically secure random codes
            # 8 characters = ~37 bits of entropy per code
            part1 = "".join(secrets.choice(charset) for _ in range(4))
            part2 = "".join(secrets.choice(charset) for _ in range(4))
            codes.append(f"{part1}-{part2}")

        # Verify uniqueness (extremely unlikely but good practice)
        if len(set(codes)) != len(codes):
            logger.warning("Duplicate backup codes generated, regenerating")
            return self._generate_backup_codes()

        return codes

    async def send_2fa_recovery_email(
        self, user: User, recovery_method: str = "password_reset"
    ) -> bool:
        """
