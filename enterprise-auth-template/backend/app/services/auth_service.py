...
# Security functions from main security module
from app.core.security import (
...
class AuthService:
    ...
    def __init__(
        ...
        self.session_security = SessionSecurityManager(session, self.cache_service)
    ...
    async def register_user(
        ...
        try:
            ...
            try:
                # Try enhanced email service first, fallback to basic if needed
                try:
    ...
    async def authenticate_user(
        ...
        try:
            ...
            #     await self._handle_failed_login(
            #         email, "Email not verified", ip_address, user_agent, user
            #     )
            ...
            # Determine privilege level for session security
            privilege_level = self._calculate_privilege_level(role_names)

            # Create secure session with fixation prevention
            session_id, session_metadata = await self.session_security.create_secure_session(
                user_id=str(user.id),
    ...
    async def request_password_reset(
        ...
        """
        try:
            # Enhanced security validation for password reset requests
            security_allowed, security_info = await password_reset_security.validate_reset_request(
                email=email,
                ip_address=ip_address or "unknown",
                user_agent=user_agent or "unknown",
                additional_context={
            ...
            # Apply security delay if needed
            if security_info.get("delay_seconds", 0) > 0:
                import asyncio
                delay_seconds = min(security_info["delay_seconds"], 300)  # Cap at 5 minutes
                logger.warning(
                    "Applying security delay to password reset request",
                    email=email,
                    ip_address=ip_address,
            ...
            # If security validation fails, log but still return success for security
            if not security_allowed:
                logger.warning(
                    "Password reset request blocked by security",
                    email=email,
                    ip_address=ip_address,
            ...
            if user and user.is_active:
                ...
                reset_token = await password_reset_security.generate_secure_reset_token(
                    user_id=str(user.id),
                    email=email
                )
    ...
    async def reset_password(
        ...
        """
        try:
            # Enhanced security validation for password reset token
            token_valid, validation_info = await password_reset_security.validate_reset_token(
                token=reset_token,
                ip_address=ip_address or "unknown",
                user_agent=user_agent or "unknown"
            )
            ...
            if not token_valid:
                logger.warning(
                    ...
                    threat_level=validation_info.get("threat_level"),
                    ip_address=ip_address
                )
                raise AuthenticationError("Invalid or compromised reset token")
    ...
    def _calculate_privilege_level(self, roles: List[str]) -> PrivilegeLevel:
        """
        Calculate privilege level from user roles for session security.

        Args:
            roles: List of role names

        Returns:
            PrivilegeLevel: Calculated privilege level
        """
