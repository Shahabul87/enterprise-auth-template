...
class OAuthService:
    ...
    def get_authorization_url(self, provider: str, state: Optional[str] = None) -> str:
        ...
        # Build authorization URL manually since we're not using Starlette request
        config = getattr(OAuthConfig, provider.upper())
    ...
    async def authenticate_oauth_user(
        ...
        try:
            ...
            logger.error("Unexpected OAuth error", provider=provider, error=str(e))
            raise HTTPException(
    ...
    async def _get_oauth_user_info(self, provider: str, code: str) -> OAuthUserInfo:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if token_response.status_code != 200:
                raise OAuthError(
                    f"Failed to exchange code for token: {token_response.text}"
                )
    ...
    async def link_oauth_account(
        self, user: User, provider: str, oauth_user_info: OAuthUserInfo
    ) -> User:
        """
    ...
    async def unlink_oauth_account(self, user: User, provider: str) -> User:
        """
        ...
        other_oauth = any(
            getattr(user, field) for field in oauth_providers if field != oauth_field
        )
