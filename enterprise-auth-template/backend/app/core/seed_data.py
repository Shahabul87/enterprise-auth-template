...
SYSTEM_ROLES = {
    ...
    "moderator": {
        ...
        "permissions": ["users:read", "content:*", "audit:read"],
        "is_system": True,
    },
    "user": {
        "description": "Regular user with basic access",
        "permissions": ["profile:*", "content:read", "content:create"],
        "is_system": True,
    },
    "guest": {
...
async def create_admin_user(session: AsyncSession, roles: dict[str, Role]) -> User:
    ...
    if not admin_user:
        ...
        logger.info("Created admin user", email=admin_email, user_id=str(admin_user.id))
...
async def create_test_user(session: AsyncSession, roles: dict[str, Role]) -> User:
    ...
    if not test_user:
        ...
        logger.info("Created test user", email=user_email, user_id=str(test_user.id))
