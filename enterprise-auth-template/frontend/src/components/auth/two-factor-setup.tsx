...
export function TwoFactorSetup({ onComplete, onCancel }: TwoFactorSetupProps) {
  ...
  const startSetup = async () => {
    ...
    try {
      const response = await apiClient.post<TwoFactorSetupData>('/api/v1/auth/2fa/setup');
  ...
  const verifyAndEnable = async () => {
    ...
    try {
      await apiClient.post('/api/v1/auth/2fa/enable', { code: verificationCode });
      setStep('backup');
    } catch (err: unknown) {
      const error = err as { response?: { data?: { detail?: string } } };
      ...
      <DialogContent className='max-w-2xl max-h-[90vh] overflow-y-auto'>
              ...
              <div className='flex items-start gap-3'>
                ...
                <div>
                  ...
                  <p className='text-sm text-muted-foreground'>
                    Protect your account from unauthorized access even if your password is
                    compromised
                  </p>
                </div>
              </div>
          ...
          <Card>
            ...
            <CardContent className='space-y-4'>
              <div className='flex justify-center p-4 bg-white rounded-lg'>
                ...
                <img src={setupData.qr_code} alt='2FA QR Code' className='max-w-[200px]' />
              </div>

              <div className='space-y-2'>
                <Label>Can&apos;t scan? Enter this code manually:</Label>
                <div className='flex gap-2'>
                  <Input value={setupData.manual_entry_key} readOnly className='font-mono' />
                  <Button
                    size='icon'
                    variant='outline'
                ...
                {copiedSecret && <p className='text-sm text-green-600'>Copied to clipboard!</p>}
              </div>
            </CardContent>
            <CardFooter>
        ...
        {step === 'backup' && setupData && (
          <Card>
            ...
            <CardFooter>
              <Button className='w-full' onClick={completeSetup} disabled={!savedBackupCodes}>
                Complete Setup
              </Button>
            </CardFooter>
          </Card>
