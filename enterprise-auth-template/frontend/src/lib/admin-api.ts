...
import { User, Role, Permission, AuditLog, ApiResponse, PaginatedResponse } from '@/types';
import {
  UserFilters,
  UserStats,
  CreateUserRequest,
...
export class AdminAPI {
  ...
  static async assignRoleToUser(userId: string, roleId: string): Promise<ApiResponse<User>> {
    return apiClient.post<User>(`/api/v1/admin/users/${userId}/roles/${roleId}`);
  }

  static async removeRoleFromUser(userId: string, roleId: string): Promise<ApiResponse<User>> {
  ...
  static async updateRole(roleId: string, roleData: UpdateRoleRequest): Promise<ApiResponse<Role>> {
  ...
  static async updatePermission(
    ...
    return apiClient.patch<Permission>(`/api/v1/admin/permissions/${permissionId}`, permissionData);
  }
  ...
  static async getBackupHistory(): Promise<
    ApiResponse<
      Array<{
        backup_id: string;
        created_at: string;
  ...
  static async restoreDatabase(backupId: string): Promise<
    ApiResponse<{
      restore_id: string;
